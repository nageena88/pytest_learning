class FitnessLog:
    ...
    def log_activity(self, kind, start_time, end_time):
        over_lap = self.overlapping_entry(start_time, end_time)
        if self.validate_entry(start_time, end_time) and not over_lap:
    ...
    def overlapping_entry(self, start_time, end_time):
        if self._activities == []:
            ...
            for exercise in self._activities:
                ...
                not_overlapping = (start_time < logged_end) and (
                    end_time > logged_start
                )
    ...
    def delete_activity(self, kind, start_time, end_time):
        for idx, activity in enumerate(self._activities):
            if (
                activity[0] == kind
                and activity[1] == start_time
                and activity[2] == end_time
            ):
